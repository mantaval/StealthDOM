...
async def send(ws, action, _bridge_timeout=15, **kwargs):
    ...
    msg = {"action": action, "_timeout": _bridge_timeout, **kwargs}
...
async def ensure_google_voice_tab(ws):
    """Find or open a Google Voice tab and switch to it."""
    ...
    result = await send(ws, "navigate", url=GOOGLE_VOICE_BASE)
    return result.get("success", False)


async def navigate_to_conversation(ws, number):
    ...
    await send(ws, "navigate", url=url)
    ...
    return await start_new_conversation(ws, normalized)


async def start_new_conversation(ws, number):
    """Start a new conversation by clicking 'Send new message' and entering the number."""
    # Click the "Send new message" button
    result = await send(ws, "click", selector="div.threads-button")
    if not result.get("success"):
    ...
    await send(ws, "click", selector=input_selector)
    await send(ws, "type", selector=input_selector, text=number)
    await asyncio.sleep(1)

    # Press Enter to confirm the recipient
    await send(ws, "keyPress", key="Enter")
    await asyncio.sleep(1)
...
async def send_sms(ws, message):
    """Type a message and click send."""
    # Click the message input to focus it
    await send(ws, "click", selector="textarea.message-input")
    await asyncio.sleep(0.3)

    # Type the message
    result = await send(ws, "type", selector="textarea.message-input", text=message)
    if not result.get("success"):
        return False
...
async def main():
    ...
    try:
        ...
        if not await ensure_google_voice_tab(ws):
        ...
        if not await navigate_to_conversation(ws, phone_number):
            print("[X] Could not open conversation.")
            return

        # Step 3: Send the message
        print(f"\n[3] Sending message...")
        if await send_sms(ws, message):
            print(f"\n{'=' * 60}")
